...
<div class="lexemes duplicates">
  ...
  <p>
    ...
    if ($cnt == 1) {
      echo __('%d entries for this lemma', count($duplicates[0]['value']['ids']));
    } else {
      $total = 0;
      foreach ($duplicates as $d) {
        $total += count($d['value']['ids']);
      }
      echo __('%s lemmas found with multiple entries', sprintf('<span class="badge">%d</span>', $cnt)).'; ';
      echo __('%s lemmas total', sprintf('<span class="badge">%d</span>', $total)).'.';
    }
    echo '<br/>';
    ...
    if (@$queryObj->query) {
      ...
      //echo __('Showing entries %d to %d', $start+1, $stop+1) . '. ';
    }
    ?>
  </p>
  ...
  <form role="search" class="form-inline" action="<?php echo $this->Html->url('/lexemes/duplicates') ?>">
    <div class="input-group">
      ...
      <div class="input-group-btn">
        ...
        <a href="<?php echo $this->Html->url('/lexemes/duplicates') ?>" class="btn btn-link">Clear</a>
      </div><!-- input-group-btn -->
    </div><!-- input-group -->
  </form>
  </div>
...
$(document).ready(function(){
  ...
  $('a.merge').click(function(){
    ...
    if (selected.length == 0) {
      alert(Gabra.i18n.merge_no_selection);
      return false;
    } else if (selected.length > 1) {
      return confirm(Gabra.i18n.merge_confirm);
    }
  });
